...
- Hero: full-bleed 4-PORTRAIT COLLAGE, left to right: `nauman-and-kids-1` · `nauman-in-shigar-with-kids` · `nauman-and-kids-2` · `prado-and-kids`. Phone: 2×2 grid, same order. Standard 1600px assets (tiles are quarter-width — sharp without a hero tier). Per-tile crops screenshot-verified at both widths; the two right tiles anchor `center 75%` so their low-sitting subjects survive the phone crop.
- **Kids block:** contain-on-blur SLIDESHOW (compact), 3 slides in order: `ejju-and-m-hassan` (caption names M Hassan), `hassnain-arsalan-and-broko` (caption names Arsalan), `grail-in-sildi-with-dog-and-puppy`. Captions ride under the stage, attached to their slides. Lede opens "Ride through Shigar and Kharmang…". No photo appears twice on the page.
