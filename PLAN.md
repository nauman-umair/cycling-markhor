...
- Hero: full-bleed 3-TILE COLLAGE — vertical `nauman-and-kids-1` · horizontal `grail-in-sildi-with-dog-and-puppy` in a wider centre column (~43%, so the dogs shot sits close to its natural shape) · vertical `nauman-in-shigar-with-kids`. Phone: the escort tile full-width on top; below, the dogs tile (wider, 3:2 split) beside the Shigar tile. Standard 1600px assets. Per-tile crops screenshot-verified at both widths — bike, puppy and the big dog all in frame.
- **Kids block:** contain-on-blur SLIDESHOW (compact), 3 slides in order: `ejju-and-m-hassan` (caption names M Hassan), `hassnain-arsalan-and-broko` (caption names Arsalan), `prado-and-kids`. Captions ride under the stage, attached to their slides. Lede opens "Ride through Shigar and Kharmang…". No photo appears twice on the page. (`nauman-and-kids-2` benched.)
